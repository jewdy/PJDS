...
import { FaEnvelope, FaWhatsapp, FaTelegram, FaLinkedin } from 'react-icons/fa';
...
const ContactMe = () => {
  ...
  const handleCopyEmail = () => {
    navigator.clipboard.writeText('pjoedesantos@gmail.com')
      .then(() => {
        setCopyPopupVisible(true); // Show the popup for email copy success
      })
  ...
  return (
    <div className="contact-container">
      ...
      {copyPopupVisible && <Popup message="Email copied to clipboard!" onClose={handleCloseCopyPopup} duration={1000} />} {/* Popup for email copy with custom duration */}
    </div>
  )
};
